...
df["customer_id"] = df["customer_id"].apply(lambda x: "" if pd.isnull(x) or str(x).strip().lower() in {"nan", "<na>", ""} else str(x).strip())

# Clean up description to remove tab, newline, and carriage return characters which could break TSV format
df["description"] = df["description"].apply(lambda x: str(x).replace("\t", " ").replace("\n", " ").replace("\r", " ") if not pd.isnull(x) else "")
...
conn = pymssql.connect(server=db_host, user=db_user, password=db_password, port=db_port, database="master", autocommit=True)
cursor = conn.cursor()
...
conn = pymssql.connect(server=db_host, user=db_user, password=db_password, port=db_port, database=db_name, autocommit=True)
cursor = conn.cursor()
